...
// FORMAT DATA
// ======================================================
// API gốc là API LỊCH SỬ, trả về nhiều phiên cùng lúc (data: [...]), mới nhất ở đầu mảng.
// Chỉ lấy đúng: phien, xuc_xac (3 viên), tong, ket_qua.
// KHÔNG lấy du_doan, loai_cau, do_tin_cay có sẵn trong API gốc (thuật toán tự dự đoán riêng).
function normalizeData(data) {
    if (!Array.isArray(data)) data = [data];
    let normalized = data.map(item => {
        const d1 = item.Xuc_xac_1 ?? item.xuc_xac_1 ?? item.x1 ?? 0;
        const d2 = item.Xuc_xac_2 ?? item.xuc_xac_2 ?? item.x2 ?? 0;
        const d3 = item.Xuc_xac_3 ?? item.xuc_xac_3 ?? item.x3 ?? 0;
        const tong = item.Tong ?? item.tong ?? item.total ?? (d1 + d2 + d3);
        const ketQuaRaw = item.Ket_qua ?? item.ket_qua ?? item.result ?? (tong >= 11 ? "tài" : "xỉu");
        const ketQua = ketQuaRaw.toString().toLowerCase();
    ...
    // API gốc trả về MỚI NHẤT Ở ĐẦU (phien giảm dần) -> đảo lại thành CŨ -> MỚI (tăng dần theo phien)
    // để toàn bộ thuật toán (dùng h[h.length-1] = phiên mới nhất) chạy đúng trên TẤT CẢ phiên có trong API.
    normalized.sort((a, b) => a.phien - b.phien);
...
// FIX BIAS: khi prob đúng bằng 0.5 (hòa tuyệt đối), KHÔNG được mặc định về 'X'.
// Random 50/50 thật sự để không kéo lệch toàn hệ thống về 1 phía khi dữ liệu mơ hồ.
function tieBreak(prob) {
    if (prob > 0.5) return 'T';
    if (prob < 0.5) return 'X';
...
function diceSumLayer(h) {
    ...
    let sc = 0, st = 0; // sc = số lần tổng này từng xuất hiện, st = số lần phiên SAU ĐÓ là Tài
    for (let i = 0; i < h.length - 1; i++) {
        let s = (h[i].xuc_xac_1 || 0) + (h[i].xuc_xac_2 || 0) + (h[i].xuc_xac_3 || 0);
        if (s === sum && i + 1 < h.length) {
            sc++;
            if ((h[i + 1].result || '') === 'Tài') st++;
        }
    }
    if (sc >= 5) {
        let prob = st / sc;
        return { p: tieBreak(prob), c: 50 + Math.abs(prob - 0.5) * 35, w: 8 };
    }
    return null;
}
...
function diceTripleLayer(h) {
    ...
    if (tc >= 3) {
        let prob = tt / tc;
        return { p: tieBreak(prob), c: 50 + Math.abs(prob - 0.5) * 70, w: 9 };
    }
    return null;
}
...
function dicePairLayer(h) {
    ...
    if (pc >= 5) {
        let prob = pt / pc;
        return { p: tieBreak(prob), c: 50 + Math.abs(prob - 0.5) * 50, w: 7 };
    }
    return null;
}
...
function diceHighLowLayer(h) {
    ...
    if (hlc >= 5) {
        let prob = hlt / hlc;
        return { p: tieBreak(prob), c: 50 + Math.abs(prob - 0.5) * 40, w: 6 };
    }
    return null;
}
...
function pattern3Layer(h) {
    ...
    if (total >= 5) {
        ...
        return { p: tieBreak(probT), c: 50 + Math.abs(probT - 0.5) * 80, w: 8 };
    }
    return null;
}
...
function pattern5Layer(h) {
    ...
    if (total >= 3) {
        let probT = nextCounts.T / total;
        return { p: tieBreak(probT), c: 50 + Math.abs(probT - 0.5) * 60, w: 6 };
    }
    return null;
}
...
function knnPatternLayer(h) {
    ...
    let probT = tCount / k;
    if (k >= 3) return { p: tieBreak(probT), c: 50 + Math.abs(probT - 0.5) * 60, w: 6 };
    return null;
}
...
function ensembleWeightedLayer(predictions) {
    ...
    return { p: tieBreak(probT), c: Math.abs(probT - 0.5) * 2 * 100, w: 5 };
}
...
app.get("/taixiu", async (req, res) => {
    try {
        ...
        res.json({ id: "AnhHadesdz Sunwin", phien_truoc: 0, xuc_xac1: 0, xuc_xac2: 0, xuc_xac3: 0, tong: 0, ket_qua: "tài", pattern: "[Đang kết nối...]", phien_hien_tai: 0, du_doan: "tài", do_tin_cay: "52%" });
    }
});
